...
public class CinemaTest {
	...
	public void testAlterarCinemaSucesso() {
		...
		int id = 0;
		Endereco end = CriarObjetos.criarEndereco();//Cria o endereço para alterar o cinema.
		...
		cineAlterar.setEndereco(end);// Não inseria um endereço ao cinema, lançando assim a exception CampoObrigatorioInexistenteException.
		...
		try {
			...
			fail();
		} catch (CinemaNaoEncontradoException e) {//Não previa essa exception.
			fail();
		}
	...
	public void testRemoverCinemaComIngresso() {
		...
		try {
			...
			fail();
		} catch (HorarioNaoPermitidoException e) {//Não previa essa exception.
			fail();
		}
